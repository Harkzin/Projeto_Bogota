...
import static web.support.api.RestAPI.jsonMapper;
...
public class ConsultaCPFMSISDN {
    ...
    private static Massas loadMassas() throws IOException {
        return jsonMapper.readValue(Files.readAllBytes(Paths.get(BASE_FILE_PATH)), Massas.class);
    }
    ...
    private static void saveMassas(Massas massas) {
        try {
            jsonMapper.writeValue(new File(BASE_FILE_PATH), massas);
        } catch (IOException e) {
