...
public class FormaPagamentoPage {
    ...
    //    Alterar após incluir id no botão Continuar
    ...
    public void preencheCartaoCreditoControleFacil(String numero, String validade, String cvv) {
        driverWeb.waitElementPresence("//*[@id='ifrm-theme-4']", 5);
        driverWeb.getDriver().switchTo().frame("ifrm-theme-4");
        preencherCartaoCreditoControleFacil("//*[@id='pan']", numero);
        preencherCartaoCreditoControleFacil("//*[@id='expiration']", validade);
        preencherCartaoCreditoControleFacil("//*[@id='cvv']", cvv);

        driverWeb.getDriver().switchTo().defaultContent();
    }

    private void preencherCartaoCreditoControleFacil(String selector, String valor) {
        WebElement campo = driverWeb.waitElementPresence(selector, 10);
        campo.sendKeys(valor);
    }

    public void clicarFinalizarTelaControleFacil() {
        driverWeb.javaScriptClick(driverWeb.findByXpath("//button[normalize-space()='Finalizar']"));
    }
}