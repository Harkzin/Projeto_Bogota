    ...
    public void validarDados(CartOrder cart) {
        ...
        if (cart.getClaroChip().getChipType() == ESIM) {
            if (cart.isDeviceCart()) {
                driverWeb.findByXpath("/html/body/main/div[3]/div/div[2]/div/div/div/div/div[1]/div[5]/div[2]/p").getText().equals("Assim que o seu pedido for entregue, você receberá o código do eSIM por e-mail." +
                        "Além disso, você terá acesso às instruções passo a passo para habilitar o eSIM em Entrar > Acompanhar Pedidos eSIM > Gerenciar eSIM");
            }
            else {
